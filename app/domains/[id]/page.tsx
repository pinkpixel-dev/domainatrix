...
export default async function DomainDetailPage({ params }: DomainDetailPageProps) {
  ...
  const hasEnrichedHost = Boolean(domain.host.isp || domain.host.region);
    ...
    <main className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <DetailPanel
          icon={ShieldCheck}
          label="Status"
          value={domain.monitoringStatus}
          detail="Based on expiry and host data"
        />
        <DetailPanel
          icon={Dna}
          label="DNS"
        ...
        {hasEnrichedHost && domain.host.isp ? (
          <DetailPanel
            icon={Network}
            label="Network"
            value={domain.host.isp}
            detail={domain.host.asNumber || ""}
          />
        ) : null}
      </section>
      ...
      {/* SSL extended fields — shown after enrichment */}
      {hasEnrichedSsl ? (
        <section>
            ...
            <dl className="divide-y divide-border text-sm">
              {domain.ssl.subject ? (
                <div className="flex items-start gap-4 px-4 py-3">
                  <dt className="w-36 shrink-0 text-muted-foreground">Subject</dt>
                  <dd className="font-mono">{domain.ssl.subject}</dd>
                </div>
              ) : null}
              {domain.ssl.issuer && domain.ssl.issuer !== "Unknown" ? (
                <div className="flex items-start gap-4 px-4 py-3">
                  <dt className="w-36 shrink-0 text-muted-foreground">Issuer</dt>
                  <dd>{domain.ssl.issuer}{domain.ssl.issuerCountry ? ` (${domain.ssl.issuerCountry})` : ""}</dd>
                </div>
              ) : null}
              {domain.ssl.validFrom ? (
                <div className="flex items-start gap-4 px-4 py-3">
                  <dt className="w-36 shrink-0 text-muted-foreground">Valid from</dt>
                  <dd>{domain.ssl.validFrom}</dd>
                </div>
              ) : null}
              {domain.ssl.validTo && domain.ssl.validTo !== "Unknown" ? (
                <div className="flex items-start gap-4 px-4 py-3">
                  <dt className="w-36 shrink-0 text-muted-foreground">Valid to</dt>
                  <dd>{domain.ssl.validTo}</dd>
                </div>
              ) : null}
              {domain.ssl.keySize ? (
                <div className="flex items-start gap-4 px-4 py-3">
                  <dt className="w-36 shrink-0 text-muted-foreground">Key size</dt>
                  <dd>{domain.ssl.keySize} bits</dd>
                </div>
              ) : null}
              {domain.ssl.fingerprint ? (
                <div className="flex items-start gap-4 px-4 py-3">
                  <dt className="w-36 shrink-0 text-muted-foreground">Fingerprint</dt>
